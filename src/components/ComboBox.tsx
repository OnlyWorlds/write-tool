...
export function ComboBox({ value, onChange, options, placeholder, className, disabled }: ComboBoxProps) {
  ...
  return (
    <div ref={wrapperRef} className="relative">
      ...
      {isOpen && (
        <div className="absolute z-10 w-full mt-1 bg-white dark:bg-dark-bg-tertiary border border-gray-200 dark:border-dark-bg-border rounded-md shadow-lg max-h-60 overflow-auto">
          {filteredOptions.length > 0 ? (
            filteredOptions.map((option) => (
              <button
                ...
                className="w-full px-3 py-2 text-left text-gray-700 dark:text-gray-200 hover:bg-blue-50 dark:hover:bg-dark-bg-hover focus:bg-blue-50 dark:focus:bg-dark-bg-hover focus:outline-none"
              >
                {option}
              </button>
            ))
          ) : options.length > 0 ? (
            <div className="px-3 py-2 text-gray-500 dark:text-gray-400 text-sm bg-gray-50 dark:bg-dark-bg-secondary">
