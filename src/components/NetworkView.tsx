...
export function NetworkView({ selectedElement, className = '' }: NetworkViewProps) {
  ...
  const paintNode = useCallback((node: GraphNode, ctx: CanvasRenderingContext2D) => {
    ...
    ctx.fillStyle = theme === 'dark' 
      ? (isHighlighted ? '#d1d5db' : '#9ca3af')
      : (isHighlighted ? '#374151' : '#6b7280');
    ...
    if (isHighlighted) {
      ...
      ctx.fillStyle = theme === 'dark' ? 'rgba(30, 41, 59, 0.95)' : 'rgba(249, 250, 251, 0.95)';
      ctx.strokeStyle = theme === 'dark' ? 'rgba(71, 85, 105, 0.3)' : 'rgba(0, 0, 0, 0.1)';
      ctx.lineWidth = 1;
      const padding = 4;
    ...
    <div className={`${className} bg-white dark:bg-dark-bg-secondary rounded-lg shadow-sm relative overflow-hidden`} ref={containerRef}>
      ...
      <div className="w-full h-full flex items-center justify-center overflow-hidden force-graph-container">
        <ForceGraph2D
          ref={fgRef}
          graphData={graphData}
          backgroundColor={theme === 'dark' ? '#0f172a' : undefined}
          nodeRelSize={1}
      ...
      <div className="absolute bottom-16 left-4 bg-slate-100/95 dark:bg-dark-bg-tertiary/95 backdrop-blur-sm rounded-lg p-3 shadow-lg max-h-48 overflow-y-auto">
        <div className="flex items-center justify-between mb-2">
          <h4 className="text-xs font-semibold text-gray-700 dark:text-gray-300">Relationships</h4>
            ...
            return Array.from(allRelationshipTypes).sort().map(type => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-2 px-1 py-0.5 rounded hover:bg-gray-200 dark:hover:bg-dark-bg-hover transition-colors ${
                    isActive ? '' : 'opacity-40'
                  }`}
                >
                  <div 
      ...
      <div className="absolute top-4 right-4 flex flex-col gap-2 z-10">
        ...
        <button
          ...
          className="bg-blue-100/90 dark:bg-blue-900/30 backdrop-blur-sm rounded-lg px-4 py-2 shadow-lg hover:bg-blue-200/90 dark:hover:bg-blue-900/40 transition-colors flex items-center gap-2 text-blue-900 dark:text-blue-200 border border-blue-200 dark:border-blue-700/50"
        >
