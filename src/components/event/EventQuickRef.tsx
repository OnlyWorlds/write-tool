      ...
      <div className="flex items-center gap-1">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="flex-1 flex items-center text-sm font-medium text-gray-900 dark:text-gray-200 group-hover:text-orange-600 dark:group-hover:text-orange-400">
...
// Event's Involves section field mapping
...
export function EventQuickRef({ event, onElementInsert, onElementUnlink }: EventQuickRefProps) {
  ...
  // Check which elements are mentioned in the history text (either as links or plain text)
  const elementsInText = useMemo(() => {
    const textIds = new Set<string>();
    const historyText = event.history || '';
    
    if (!historyText) return textIds;
    
    // Check each linked element to see if it appears in the history text
    linkedElements.forEach(({ element }) => {
      // Check for markdown links
      const linkPattern = new RegExp(`\\[[^\\]]*\\]\\([^):]+:${element.id}\\)`, 'g');
      if (historyText.match(linkPattern)) {
      ...
      if (historyText.match(namePattern)) {
        textIds.add(element.id);
      }
    });
    
    return textIds;
  }, [event.history, linkedElements]);
  ...
  const elementMentionCounts = useMemo(() => {
    ...
    linkedElements.forEach(({ element }) => {
      ...
      const linkPattern = new RegExp(`\\[[^\\]]*\\]\\([^):]+:${element.id}\\)`, 'g');
      const linkMatches = historyText.match(linkPattern);
      if (linkMatches) {
      ...
      const namePattern = new RegExp(`(?<![a-zA-Z0-9])${nameEscaped}(?![a-zA-Z0-9])`, 'gi');
      const nameMatches = historyText.match(namePattern);
      if (nameMatches) {
        ...
        <div className="flex items-center justify-between text-xs">
          <div className="flex items-center gap-3">
            ...
            <span className="text-green-600 dark:text-green-400">
              In history: <span className="font-medium">{stats.inText}</span>
            </span>
            {/* <span className="text-gray-500 dark:text-gray-400">
              Not in history: <span className="font-medium">{stats.notInText}</span>
            </span> */}
          </div>
        </div>
      </div>
      ...
      <div className="p-2 bg-white dark:bg-dark-bg-tertiary border-b border-gray-200 dark:border-dark-bg-border">
        <div className="flex gap-2">
          ...
          >
            ...
            <option value="in-text">Show in history</option>
            <option value="not-in-text">Show not in history</option>
          </select>
      ...
      <div className="flex-1 overflow-y-auto p-3">
        {filteredElements.length === 0 ? (
          <p className="text-sm text-gray-500 dark:text-gray-400 text-center py-8">
            {filterMode === 'in-text' 
              ? 'No involved elements found in history'
              : filterMode === 'not-in-text'
                ? 'All involved elements are in history'
                : 'No elements involved in this event yet'}
          </p>
        ) : (
