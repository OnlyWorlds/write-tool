...
import { EventQuickRef } from './EventQuickRef';
import { EnhancedHistoryEditor, type EnhancedHistoryEditorRef } from './EnhancedHistoryEditor';
import './EventWriter.css';
import { EventWritingStats } from './EventWritingStats';

interface EventWriterProps {
  element: Element;
}

export function EventWriter({ element }: EventWriterProps) {
  const { saveElement, updateElement } = useWorldContext();
  
  // Check for draft content in localStorage
  const getDraftContent = () => {
    const draft = localStorage.getItem(`event_draft_${element.id}`);
    if (draft && draft !== element.history) {
      return draft;
    }
  ...
  const [currentElement, setCurrentElement] = useState({ ...element, history: draftContent });
  ...
  const editorRef = useRef<EnhancedHistoryEditorRef>(null);
  const detectionWidgetRef = useRef<HTMLButtonElement>(null);

  const handleSave = async (newContent: string) => {
    const success = await saveElement(currentElement.id, { history: newContent });
    if (success) {
      // Clear draft on successful save
      localStorage.removeItem(`event_draft_${currentElement.id}`);
      setIsDirty(false);
    }
    return success;
  };
  ...
  const handleElementUnlink = async (elementId: string, elementType: string) => {
    ...
    // Save the field update (no need to modify history content since we're using visual indicators only)
  ...
  const handleRevertChanges = () => {
    if (confirm('Are you sure you want to revert to the last saved version? All unsaved changes will be lost.')) {
      const originalContent = element.history || '';
      setContent(originalContent);
      setCurrentElement(prev => ({ ...prev, history: originalContent }));
      if (editorRef.current) {
        editorRef.current.setContent(originalContent);
      }
      localStorage.removeItem(`event_draft_${element.id}`);
      setIsDirty(false);
    }
  };
  ...
  useEffect(() => {
    ...
    if (draft && draft !== element.history) {
      ...
      setCurrentElement(prev => ({ ...prev, history: draft }));
      setContent(draft);
      ...
      // No draft, use element's history
      setCurrentElement(prev => ({ ...prev, history: element.history || '' }));
      setContent(element.history || '');
      if (editorRef.current) {
        editorRef.current.setContent(element.history || '');
      }
    }
  ...
  const handleLinkAll = async () => {
    ...
    if (success) {
      // Don't modify history content or dirty state since we're only updating fields
    }
    
    // Refresh highlights after linking
    if (highlightsEnabled) {
      setTimeout(() => editorRef.current?.refreshHighlights(), 100);
    }
  };
  
  // Helper to get the field name for a category - Event's Involves section fields
    ...
    <div className={`event-editor-wrapper bg-white dark:bg-dark-bg-primary flex flex-col ${isFullscreen ? 'fixed inset-0 z-50' : 'relative h-[600px]'}`}>
      ...
      <div className="flex-shrink-0 flex items-center justify-between px-6 py-2 border-b border-gray-200 dark:border-dark-bg-border bg-gradient-to-r from-green-50 to-blue-50 dark:from-dark-bg-secondary dark:to-dark-bg-tertiary shadow-sm">
        <div className="flex items-center gap-4">
          <EventWritingStats content={content} />
        ...
        <div className="flex items-center gap-2">
          {/* Highlight controls as checkbox with refresh icon on left */}
          <div className="flex items-center gap-1">
            {highlightsEnabled && (
              <button
                onClick={() => {
            ...
            <label className="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400 cursor-pointer">
              <input
                ...
                className="w-3 h-3 appearance-none border border-gray-300 dark:border-gray-600 rounded focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-600 focus:ring-offset-0 checked:bg-blue-600 checked:border-blue-600 dark:checked:bg-blue-600 dark:checked:border-blue-600"
              />
          ...
          <div className="flex items-center gap-2">
            <label className="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400 cursor-pointer">
              <input
                ...
                className="w-3 h-3 appearance-none border border-gray-300 dark:border-gray-600 rounded focus:ring-2 focus:ring-green-500 dark:focus:ring-green-600 focus:ring-offset-0 checked:bg-green-600 checked:border-green-600 dark:checked:bg-green-600 dark:checked:border-green-600"
              />
              <span>auto-save</span>
            </label>
          </div>
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          <EnhancedHistoryEditor
            ...
            onContentChange={(newContent) => {
              ...
              localStorage.setItem(`event_draft_${currentElement.id}`, newContent);
              // Check if content is different from saved version
              setIsDirty(newContent !== element.history);
              // Also update currentElement's history so EventQuickRef sees the changes
              setCurrentElement(prev => ({ ...prev, history: newContent }));
            }}
